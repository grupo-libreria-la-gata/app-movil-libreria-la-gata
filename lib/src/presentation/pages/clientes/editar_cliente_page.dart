...
class _EditarClientePageState extends State<EditarClientePage> {
  ...
  Future<void> _actualizarCliente() async {
    ...
    try {
      ...
      final response = await _service.actualizarCliente(
        widget.cliente.clienteId,
        request,
      );

      if (response.success) {
        _mostrarExito(response.message ?? 'Cliente actualizado exitosamente');
        Navigator.of(context).pop(true);
      } else {
        _mostrarError(response.message ?? 'Error al actualizar cliente');
      }
    } catch (e) {
      _mostrarError('Error al actualizar cliente: ${e.toString()}');
    } finally {
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(
                      _activo
                          ...
                          : 'El cliente no puede realizar compras',
                    ),
                    value: _activo,
                    onChanged: (value) {
                      setState(() {
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: _loading ? null : _actualizarCliente,
